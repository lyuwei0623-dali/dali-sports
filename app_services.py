...
def compose_services(*, read_only: bool = False) -> AppServices:
    data_dir = Path(os.environ.get("APP_DATA_DIR", "data"))
    if not read_only:
        data_dir.mkdir(parents=True, exist_ok=True)
    mlb_store = MLBReleaseSnapshotStore(str(data_dir / "mlb_release.sqlite3"), read_only=read_only)
    football = FootballModule.from_environment(str(data_dir / "football.sqlite3"), read_only=read_only)
    try:
