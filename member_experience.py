...
def _compact_report(report: SharedReport) -> SharedReport:
    ...
    if football:
        ...
        columns = (
            ...
            ReportColumn("model_ev", "賽果預估（非投注機率）", "normal", "14%"),
            ReportColumn("moneyline", "獨贏推薦／EV", "primary", "12%"),
            ReportColumn("spread", "讓分推薦／EV", "primary", "12%"),
            ReportColumn("total", "大小推薦／EV", "primary", "12%"),
        )
        title = "MLB｜當日賽事分析"
...
def _source_caption(summary: Any) -> str:
    ...
    sources = [
        ...
        for name in (
            ...
            "ESPN", "API-Football 聯賽強度", "football-data.org", "The Odds API",
        )
        if name in text
    ]
