...
from odds_ingestion import fetch_feed, complete_market
...
class MLBReleaseSnapshotStore:
    ...
    def _connect(self) -> sqlite3.Connection:
        return database_connection(self.db_path, "mlb", read_only=self.read_only)
    ...
    def _initialize(self) -> None:
        with self._connect() as connection:
            connection.executescript("""
                ...
                -- Durable historical evidence. Daily release snapshots are
                -- presentation records; these rows are the MLB team-history
                -- source used by future pre-game calculations.
                CREATE TABLE IF NOT EXISTS mlb_historical_games (
                    game_pk TEXT PRIMARY KEY, game_date TEXT NOT NULL,
                    home_team_id INTEGER, home_team TEXT NOT NULL,
                    away_team_id INTEGER, away_team TEXT NOT NULL,
                ...
                CREATE INDEX IF NOT EXISTS idx_mlb_history_team_date
                ON mlb_historical_games(game_date, home_team_id, away_team_id);
                CREATE TABLE IF NOT EXISTS mlb_history_sync_state (
                    season INTEGER PRIMARY KEY, completed INTEGER NOT NULL,
                    updated_at TEXT NOT NULL, last_error TEXT
                );
                CREATE TABLE IF NOT EXISTS mlb_market_history (
                    id INTEGER PRIMARY KEY AUTOINCREMENT, date_str TEXT NOT NULL,
                    event_id TEXT NOT NULL, market_type TEXT NOT NULL, side TEXT NOT NULL,
                    raw_line TEXT, decimal_price REAL NOT NULL, source TEXT NOT NULL,
                    observed_at TEXT NOT NULL
                );
                CREATE INDEX IF NOT EXISTS idx_mlb_market_history_event
                ON mlb_market_history(date_str, event_id, observed_at);
            """)

    def sync_historical_games(self, client: "MLBOfficialClient", seasons: Iterable[int]) -> dict[str, Any]:
        """Import completed MLB games once per season into durable storage.

        This protected backend operation is resumable and never runs from a
        member request. MLB's official schedule is the canonical source.
        """
        imported, completed = 0, []
        for season in sorted({int(value) for value in seasons if int(value) >= 1900}):
            with self._connect() as connection:
                state = connection.execute("SELECT completed FROM mlb_history_sync_state WHERE season=?", (season,)).fetchone()
            if state and state["completed"] and season < datetime.now(TZ_TAIPEI).year:
                continue
            try:
                games = client.completed_games_for_season(season)
                with self._connect() as connection:
                    for game in games:
                        connection.execute("""INSERT INTO mlb_historical_games VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
                          ON CONFLICT(game_pk) DO UPDATE SET home_runs=excluded.home_runs,away_runs=excluded.away_runs,
                          status=excluded.status,raw_json=excluded.raw_json,ingested_at=excluded.ingested_at""", (
                            game["game_pk"], game["game_date"], game["home_team_id"], game["home_team"],
                            game["away_team_id"], game["away_team"], game["home_runs"], game["away_runs"],
                            game["status"], "MLB Stats API", json.dumps(game["raw"], ensure_ascii=False), _taipei_now_iso()))
                        imported += 1
                    season_finished = int(season < datetime.now(TZ_TAIPEI).year)
                    connection.execute("""INSERT INTO mlb_history_sync_state VALUES(?,?,?,NULL)
                      ON CONFLICT(season) DO UPDATE SET completed=excluded.completed,updated_at=excluded.updated_at,last_error=NULL""",
                      (season, season_finished, _taipei_now_iso()))
                completed.append(season)
            except Exception:
                with self._connect() as connection:
                    connection.execute("""INSERT INTO mlb_history_sync_state VALUES(?,?,?,?)
                      ON CONFLICT(season) DO UPDATE SET updated_at=excluded.updated_at,last_error=excluded.last_error""",
                      (season, 0, _taipei_now_iso(), "official_history_source_temporarily_unavailable"))
        return {"status": "ok", "games_upserted": imported, "completed_seasons": completed}

    def team_history_profile(self, team_id: Optional[int], before: str, venue: str) -> dict[str, Any]:
        if not team_id:
            return {"games_used": 0, "source": "none"}
        with self._connect() as connection:
            rows = connection.execute("""SELECT game_date,home_team_id,away_team_id,home_runs,away_runs
              FROM mlb_historical_games WHERE game_date<? AND (home_team_id=? OR away_team_id=?)
              ORDER BY game_date DESC LIMIT 40""", (before, team_id, team_id)).fetchall()
        venue_rows = [row for row in rows if (venue == "home" and row["home_team_id"] == team_id) or (venue == "away" and row["away_team_id"] == team_id)]
        usable = venue_rows if len(venue_rows) >= 8 else rows
        if not usable:
            return {"games_used": 0, "source": "none"}
        weights = [.95 ** index for index in range(len(usable))]
        total = sum(weights)
        scored = [int(row["home_runs"] if row["home_team_id"] == team_id else row["away_runs"]) for row in usable]
        allowed = [int(row["away_runs"] if row["home_team_id"] == team_id else row["home_runs"]) for row in usable]
        return {"games_used": len(usable), "source": "official_mlb_history", "venue_specific": usable is venue_rows,
                "runs_for": sum(value * weight for value, weight in zip(scored, weights)) / total,
                "runs_against": sum(value * weight for value, weight in zip(allowed, weights)) / total}
    ...
    def save_calibrated_snapshot(
        ...
        with self._connect() as connection:
            ...
            self._capture_payload_markets(connection, date_str, payloads, "SUPER 人工校正", updated_at)
    ...
    def save_automatic_snapshot(
        ...
        *,
        ...
        # A completed game's prices disappear from The Odds API.  Preserve the
        # most recent valid pre-game decision from both the live snapshot and
        # revision history before validating and saving the replacement.
        with self._connect() as connection:
            archived_rows = _mlb_archived_recommendation_rows(connection, date_str)
        rows = _merge_mlb_automatic_rows(archived_rows, rows, timestamp)
        ...
        with self._connect() as connection:
            ...
            """, (date_str, payload_json, source_updated_at, timestamp, timestamp,
                  str(note), timestamp))
            self._capture_payload_markets(connection, date_str, rows, "The Odds API 自動基準盤", timestamp)
    ...
    def mark_automatic_no_games(
        ...
        *,
        ...
        """An empty provider response must never erase a previously saved slate."""
        return self.mark_automatic_snapshot_failed(date_str, note, now=now)
    ...
    @staticmethod
    def _capture_payload_markets(connection: sqlite3.Connection, date_str: str,
                                 payloads: Iterable[Mapping[str, Any]], source: str,
                                 observed_at: str) -> None:
        for payload in payloads:
            event_id = str(payload.get("event_id") or "")
            for recommendation in payload.get("recommendations") or ():
                try:
                    connection.execute("""INSERT INTO mlb_market_history
                      (date_str,event_id,market_type,side,raw_line,decimal_price,source,observed_at)
                      VALUES(?,?,?,?,?,?,?,?)""", (date_str, event_id, str(recommendation["market_type"]),
                        str(recommendation["side"]), recommendation.get("raw_line"), float(recommendation["decimal_price"]),
                        source, observed_at))
...
def _mlb_archived_recommendation_rows(connection: sqlite3.Connection, date_str: str) -> list[dict[str, Any]]:
    """Recover the newest revision with recommendations for every game."""
    values: list[str] = []
    current = connection.execute(
        "SELECT payload_json FROM mlb_automatic_snapshots WHERE date_str=?", (date_str,)
    ).fetchone()
    if current:
        values.append(current["payload_json"])
    values.extend(row["payload_json"] for row in connection.execute(
        """SELECT payload_json FROM mlb_automatic_snapshot_history
           WHERE date_str=? ORDER BY revision_id DESC""", (date_str,)
    ).fetchall())
...
def _mlb_recommendation_types(row: Mapping[str, Any]) -> set[str]:
...
def _merge_mlb_automatic_rows(
    ...
    for source in current_rows:
        ...
        if not previous:
            ...
            continue
        try:
            started = parse_iso(str(current.get("kickoff") or previous.get("kickoff"))) <= now
        except (TypeError, ValueError):
            started = False
        if started:
            locked = dict(previous)
            settlement = normalize_settlement_status(current.get("settlement_status"))
            if settlement != "pending":
                locked["settlement_status"] = settlement
            locked["record_state"] = "pregame_locked"
            message = "賽事已開賽：顯示開賽前最後保存的推薦與盤口"
            if message not in str(locked.get("warning") or ""):
                locked["warning"] = "｜".join(value for value in (str(locked.get("warning") or ""), message) if value)
            output.append(locked)
            continue
        previous_types = _mlb_recommendation_types(previous)
        current_types = _mlb_recommendation_types(current)
        ...
        if missing:
            ...
            message = "本次盤口資料不完整：缺少類別保留上一個有效快照，僅供歷史參考，不代表目前可下注價格"
            if message not in str(current.get("warning") or ""):
                current["warning"] = "｜".join(value for value in (str(current.get("warning") or ""), message) if value)
        output.append(current)
    for event_id, previous in archived.items():
        if event_id in seen:
            continue
        try:
            started = parse_iso(str(previous.get("kickoff"))) <= now
        except (TypeError, ValueError):
            started = False
        if started:
            locked = dict(previous)
...
def _complete_odds_market(key: str, market: Mapping[str, Any], home: str, away: str) -> bool:
    ...
    if complete_market(dict(market), home, away, mlb_team_key, football=False) is None:
        return False
...
class MLBOfficialClient:
    ...
    def completed_games_for_season(self, season: int) -> list[dict[str, Any]]:
        """Read final regular-season games for a durable model-history import."""
        payload = self.http.get_json(
            f"{MLB_API}/schedule", sportId=1, season=int(season), gameType="R",
            hydrate="linescore",
        )
        records: list[dict[str, Any]] = []
        for game_day in payload.get("dates", []):
            for raw in game_day.get("games", []):
                teams, status = raw.get("teams", {}), raw.get("status", {})
                home, away = teams.get("home", {}), teams.get("away", {})
                if status.get("abstractGameState") != "Final":
                    continue
                try:
                    records.append({"game_pk": str(raw["gamePk"]), "game_date": str(raw.get("gameDate") or ""),
                                    "home_team_id": int(home["team"]["id"]), "home_team": str(home["team"]["name"]),
                                    "away_team_id": int(away["team"]["id"]), "away_team": str(away["team"]["name"]),
                                    "home_runs": int(home["score"]), "away_runs": int(away["score"]),
                                    "status": str(status.get("detailedState") or "Final"), "raw": raw})
                except (KeyError, TypeError, ValueError):
                    continue
        return records
...
class MLBPreReleaseService:
    ...
    def __init__(self, simulations: int = 30_000, min_ev: float = .03, http: Optional[HTTP] = None,
                 history_store: Optional[MLBReleaseSnapshotStore] = None):
    ...
    def _model(self, game: Game, home: TeamFeature, away: TeamFeature, weather_multiplier: float) -> ModelOutput:
        ...
        if self.history_store is not None:
            home_profile = self.history_store.team_history_profile(game.home_team_id, game.kickoff, "home")
            away_profile = self.history_store.team_history_profile(game.away_team_id, game.kickoff, "away")
            # Historical profile is team-specific official final-game evidence.
            # Blend it conservatively with the frozen lineup/bullpen base model
            # rather than replacing the V2.0 baseball factors.
            if int(home_profile.get("games_used", 0)) >= 8 and int(away_profile.get("games_used", 0)) >= 8:
                history_home = math.sqrt(float(home_profile["runs_for"]) * float(away_profile["runs_against"]))
                history_away = math.sqrt(float(away_profile["runs_for"]) * float(home_profile["runs_against"]))
                home_lambda = .65 * home_lambda + .35 * history_home
                away_lambda = .65 * away_lambda + .35 * history_away
    ...
    def _recommend(self, model: ModelOutput, quotes: Iterable[SuperQuote]) -> list[Recommendation]:
        ...
        if not result:
            return []
        # Keep one best actionable recommendation *per market category*.
        # A member table has separate 狨贏／讓分／大小 columns; selecting one
        # global winner made the other two appear as 「無法評估」 even though
        # valid prices and calculations were already present.
        def score(item: Recommendation) -> float:
            return .70 * item.probability + .30 * ((math.tanh(item.ev * 2.0) + 1.0) / 2.0)
        winners = {
            market_type: max(items, key=lambda item: (score(item), item.ev))
            for market_type, items in {
                kind: [item for item in result if item.market_type == kind]
                for kind in ("moneyline", "spread", "total")
            }.items() if items
        }
        output = []
        for item in result:
            if winners.get(item.market_type) == item:
                category = {"moneyline": "獨贏", "spread": "讓分", "total": "大小"}[item.market_type]
                label = f"主推｜{category}價值盤" if item.ev >= self.min_ev else f"主推｜{category}穩健盤（EV 非主要優勢）"
                output.append(Recommendation(item.market_type, item.side, item.raw_line, item.decimal_price,
                                             item.probability, item.implied_probability, item.ev, True, label))
            else:
                output.append(Recommendation(item.market_type, item.side, item.raw_line, item.decimal_price,
                                             item.probability, item.implied_probability, item.ev, False, "候選盤口"))
        return output
...
class MLBAutoSnapshotRunner:
    ...
    def __init__(
        ...
        self.store = store
        self.service = service or MLBPreReleaseService(history_store=store)
    ...
    def run(
        ...
        try:
            ...
            for game in games:
                if parse_iso(game.kickoff) <= current:
                    quotes[game.event_id] = []  # Archive only after first pitch.
...
def _match_odds_market(game: Game, markets: Iterable[OddsMarket]) -> Optional[OddsMarket]:
    ...
    # Doubleheaders must not borrow odds from the other game on the same day.
    if abs((parse_iso(selected.commence_time) - kickoff).total_seconds()) > 1800:
        return None
