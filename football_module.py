...
import re
import sqlite3
from odds_ingestion import fetch_feed, select_markets, FeedError
from durable_store import database_connection
import unicodedata
...
SCORE_DECISION_VERSION = "score-decision-v18-market-calibrated"
DEFAULT_LEAGUES = {
...
class FootballModule:
    ...
    def __init__(self, config: FootballConfig, *, read_only: bool = False) -> None:
        self.config = config
    ...
    @classmethod
    def from_environment(cls, db_path: str, *, read_only: bool = False) -> "FootballModule":
    ...
    def _db(self) -> sqlite3.Connection:
        return database_connection(self.config.db_path, "football", read_only=self.read_only)
    ...
    def _init_db(self) -> None:
        with self._db() as conn:
            ...
            END;
            -- Durable team evidence.  Unlike football_events, these rows are
            -- never removed by a daily refresh and are the only source used
            -- for an historical team profile.
            CREATE TABLE IF NOT EXISTS football_historical_fixtures (
              fixture_id TEXT PRIMARY KEY, league_key TEXT NOT NULL, season INTEGER NOT NULL,
              kickoff TEXT NOT NULL, status TEXT NOT NULL,
              home_team_id TEXT, home_team TEXT NOT NULL, away_team_id TEXT, away_team TEXT NOT NULL,
              home_goals INTEGER NOT NULL, away_goals INTEGER NOT NULL,
              source TEXT NOT NULL, raw_json TEXT NOT NULL, ingested_at TEXT NOT NULL
            );
            CREATE INDEX IF NOT EXISTS football_historical_team_lookup
              ON football_historical_fixtures(league_key, kickoff, home_team, away_team);
            CREATE TABLE IF NOT EXISTS football_history_sync_state (
              league_key TEXT NOT NULL, season INTEGER NOT NULL, next_page INTEGER NOT NULL,
              completed INTEGER NOT NULL, updated_at TEXT NOT NULL, last_error TEXT,
              PRIMARY KEY(league_key, season)
            );
            -- Every automatic consensus quote is retained.  Manual prices
            -- already have football_calibration_history; this table supplies
            -- the corresponding automatic market timeline.
            CREATE TABLE IF NOT EXISTS football_market_history (
              id INTEGER PRIMARY KEY AUTOINCREMENT, date_str TEXT NOT NULL, event_id TEXT NOT NULL,
              market_type TEXT NOT NULL, side TEXT NOT NULL, line REAL, decimal_price REAL NOT NULL,
              source TEXT NOT NULL, observed_at TEXT NOT NULL
            );
            CREATE INDEX IF NOT EXISTS football_market_history_lookup
              ON football_market_history(date_str, event_id, observed_at);
            """)

    # --------------------------- backend fetch layer -------------------------
    def sync_historical_fixtures(self, seasons: Mapping[str, int], max_pages: int = 12) -> dict[str, Any]:
        """Incrementally import completed fixtures into the durable history store.

        This is an administrator-only, quota-aware bootstrap.  It deliberately
        resumes from the saved page cursor instead of attempting a large
        historical download during a member request or a normal daily refresh.
        """
        if not self.config.api_football_key:
            raise RuntimeError("API_FOOTBALL_KEY is required")
        budget = max(1, min(int(max_pages), 80))
        headers = {"x-apisports-key": self.config.api_football_key}
        imported = calls = 0
        progress: list[dict[str, Any]] = []
        with self._db() as conn:
            states = {(r["league_key"], int(r["season"])): dict(r) for r in conn.execute(
                "SELECT league_key,season,next_page,completed FROM football_history_sync_state").fetchall()}
        for league_key, league_id in DEFAULT_LEAGUES.items():
            season = seasons.get(league_key)
            if not isinstance(season, int) or season < 1900:
                continue
            state = states.get((league_key, season), {})
                ...
                try:
                    response = requests.get(f"{self.config.api_football_base}/fixtures", headers=headers,
                        params={"league": league_id, "season": season, "status": "FT", "page": page,
                                "timezone": "Asia/Taipei"}, timeout=15)
                    response.raise_for_status()
                    body = response.json()
                except Exception:
                    with self._db() as conn:
                        conn.execute("""INSERT INTO football_history_sync_state VALUES(?,?,?,?,?,?)
                          ON CONFLICT(league_key,season) DO UPDATE SET updated_at=excluded.updated_at,last_error=excluded.last_error""",
                          (league_key, season, page, 0, _now(), "history_source_temporarily_unavailable"))
                    progress.append({"league": league_key, "season": season, "status": "source_retry_needed"})
                    break
                calls += 1
                rows = body.get("response") or []
                with self._db() as conn:
                    for item in rows:
                        fixture, teams, goals = item.get("fixture", {}), item.get("teams", {}), item.get("goals", {})
                        if not fixture.get("id") or goals.get("home") is None or goals.get("away") is None:
                            continue
                        conn.execute("""INSERT INTO football_historical_fixtures VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                          ON CONFLICT(fixture_id) DO UPDATE SET status=excluded.status,home_goals=excluded.home_goals,
                          away_goals=excluded.away_goals,raw_json=excluded.raw_json,ingested_at=excluded.ingested_at""", (
                            str(fixture["id"]), league_key, season, fixture.get("date"), fixture.get("status", {}).get("short", "FT"),
                            str(teams.get("home", {}).get("id") or ""), teams.get("home", {}).get("name", ""),
                            str(teams.get("away", {}).get("id") or ""), teams.get("away", {}).get("name", ""),
                            int(goals["home"]), int(goals["away"]), "API-Football", json.dumps(item, ensure_ascii=False), _now()))
                        imported += 1
                paging = body.get("paging") or {}
                total = int(paging.get("total") or page)
                done = not rows or page >= total
                with self._db() as conn:
                    conn.execute("""INSERT INTO football_history_sync_state VALUES(?,?,?,?,?,NULL)
                      ON CONFLICT(league_key,season) DO UPDATE SET next_page=excluded.next_page,completed=excluded.completed,
                      updated_at=excluded.updated_at,last_error=NULL""", (league_key, season, page + 1, int(done), _now()))
                if done:
                    progress.append({"league": league_key, "season": season, "status": "completed"})
                    break
                page += 1
            if calls >= budget:
                break
        return {"status": "ok", "api_calls": calls, "fixtures_upserted": imported, "progress": progress}

    def _historical_team_profile(self, conn: sqlite3.Connection, league_key: str, team: str,
                                 venue: str, before: str) -> dict[str, Any]:
        rows = conn.execute("""SELECT kickoff,home_team,away_team,home_goals,away_goals FROM football_historical_fixtures
          WHERE league_key=? AND kickoff<? AND (home_team=? OR away_team=?) ORDER BY kickoff DESC LIMIT 40""",
          (league_key, before, team, team)).fetchall()
        venue_rows = [r for r in rows if (venue == "home" and r["home_team"] == team) or (venue == "away" and r["away_team"] == team)]
        usable = venue_rows if len(venue_rows) >= 6 else rows
        if not usable:
            return {"matches_used": 0, "source": "none"}
        weights = [0.94 ** index for index in range(len(usable))]
        goals_for = [int(r["home_goals"] if r["home_team"] == team else r["away_goals"]) for r in usable]
        goals_against = [int(r["away_goals"] if r["home_team"] == team else r["home_goals"]) for r in usable]
        total_weight = sum(weights)
        return {"matches_used": len(usable), "source": "historical_fixtures",
                "venue_specific": usable is venue_rows,
                "attack": round(sum(value * weight for value, weight in zip(goals_for, weights)) / total_weight, 4),
                "defence": round(sum(value * weight for value, weight in zip(goals_against, weights)) / total_weight, 4)}
    ...
    def refresh_daily_snapshot(self, date_str: str, season_by_league: Mapping[str, int]) -> dict[str, Any]:
        ...
        API-Football 聯賽強度或 The Odds API。可安全重試；event rows are
        ...
        if not self.config.api_football_key:
            ...
            try:
                ...
                diagnostics.append({"code": "external_api_error", "message": "API-Football 賽程暫時無法取得，改用 ESPN 賽程備援"})
        # Independent form data is optional enrichment.  It is deliberately
        # sourced from football-data.org rather than API-Football, so a gap in
        # one provider cannot turn every team forecast into league priors.
        independent_profiles = self._fetch_football_data_profiles(target)
        ...
        if not events:
            ...
            # A partial API-Football response must not suppress ESPN's other
            # leagues. Merge by ordered teams, league, and kickoff tolerance.
            for backup in self._espn_fallback_events(espn_index):
                known = any(e["league_key"] == backup["league_key"]
                    and (_team_key(e["home"]), _team_key(e["away"])) == (_team_key(backup["home"]), _team_key(backup["away"]))
                    and abs((_parse_timestamp(e["kickoff"]) - _parse_timestamp(backup["kickoff"])).total_seconds()) <= 1800
                    for e in events)
                if not known:
                    events.append(backup)
                    diagnostics.append({"code": "espn_partial_schedule_fallback", "message": "已補入主來源缺漏賽程"})
        ...
        with self._db() as conn:
            ...
            for event in events:
                ...
                event["independent_profile"] = {
                    "home": independent_profiles.get(_team_key(event["home"]), {}),
                    "away": independent_profiles.get(_team_key(event["away"]), {}),
                }
                history = {
                    "home": self._historical_team_profile(conn, event["league_key"], event["home"], "home", event["kickoff"]),
                    "away": self._historical_team_profile(conn, event["league_key"], event["away"], "away", event["kickoff"]),
                }
                model = self._build_base_model(event, {}, history)
                ...
                current_time = _parse_timestamp(_now())
                kickoff_time = _parse_timestamp(event["kickoff"])
                # No live/post-match price can become a pregame recommendation.
                if kickoff_time is None or current_time >= kickoff_time:
                    matched = []
                kinds = {q["market_type"] for q in matched}
                accepted = set()
                for kind in kinds:
                    newest = max(q["observed_at"] for q in matched if q["market_type"] == kind)
                    old = conn.execute("SELECT MAX(observed_at) FROM football_market_reference WHERE date_str=? AND event_id=? AND market_type=?",
                                       (date_str, event["event_id"], kind)).fetchone()[0]
                    if old and _parse_timestamp(old) and _parse_timestamp(old) > _parse_timestamp(newest):
                        continue
                    accepted.add(kind)
                    # Replace a validated market as a whole. Missing categories
                    # remain saved with their ORIGINAL observed_at timestamp.
                    conn.execute("DELETE FROM football_market_reference WHERE date_str=? AND event_id=? AND market_type=?",
                                 (date_str, event["event_id"], kind))
                matched = [q for q in matched if q["market_type"] in accepted]
                market_audit.append({"event_id": str(event["event_id"]), "home": event["home"], "away": event["away"],
                                     "matched_markets": sorted(kinds),
                                     "status": "complete" if len(kinds) == 3 else "started_or_missing_market"})
                for q in matched:
                    ...
                    conn.execute("""INSERT INTO football_market_history
                      (date_str,event_id,market_type,side,line,decimal_price,source,observed_at)
                      VALUES(?,?,?,?,?,?,?,?)""", (date_str, event["event_id"], q["market_type"], q["side"],
                        q["line"], q["price"], q["provider"], q["observed_at"]))
                stored += 1
            summary = {"api_football_events": len(api_events), "espn_events": len(espn_index),
                       ...
                       "football_data_teams": len(independent_profiles),
                       "standing_strength_teams": sum(
                           int(bool((event.get("standing_profile") or {}).get("home"))) +
                           int(bool((event.get("standing_profile") or {}).get("away"))) for event in events),
    ...
    def run_football_auto_snapshot(self, date_str: str, season_by_league: Mapping[str, int],
        ...
        try:
            ...
            attempted_at = _timestamp(now)  # Evaluation time after network acquisition.
            ...
            # Odds providers normally remove an event after kickoff.  Re-running
            # a same-day snapshot must therefore merge with the last saved
            # pre-match decision instead of replacing it with empty PASS rows.
            # History is also consulted so V19 can recover a valid pre-match
            # revision even when V18 already overwrote the current snapshot.
            with self._db() as conn:
                archived_rows = _football_archived_recommendation_rows(conn, date_str)
            rows = _merge_football_automatic_rows(archived_rows, rows, attempted_at)
    ...
    def _build_automatic_snapshot_payload(self, date_str: str, updated_at: str) -> tuple[list[dict[str, Any]], dict[str, Any], Optional[str]]:
        ...
        with self._db() as conn:
            ...
            references = conn.execute("""SELECT event_id,market_type,side,line,price,observed_at,provider
        ...
        for item in references:
            ...
            current = _parse_timestamp(updated_at)
            if observed is None or current is None or not 0 <= (current - observed).total_seconds() <= self.config.max_market_age_seconds:
                continue  # Kept in history; NEVER relabelled as a fresh price.
            entry = {"market_type": item["market_type"], "side": item["side"], "line": item["line"], "decimal_price": item["price"],
                     "observed_at": item["observed_at"], "provider": item["provider"]}
        ...
        for event in events:
            ...
            normalised = []
            # A corrupt legacy category must not discard other complete markets.
            for kind in ("moneyline", "spread", "total"):
                try:
                    group = [_validate_market(m) for m in markets if m["market_type"] == kind]
                    _validate_market_set(group)
                    normalised.extend(group)
                except (ValueError, TypeError, KeyError, OverflowError):
                    continue
            kickoff_time = _parse_timestamp(event["kickoff"])
            if kickoff_time is None or kickoff_time <= _parse_timestamp(updated_at):
                normalised = []  # Only an archived pregame decision may survive.
            if normalised:
                _validate_market_set(normalised)
                v21_recommendations = self._calculate_recommendations(model, normalised)
                calculated = self._calculate_best_recommendations(model, normalised)
                # When team evidence is still being backfilled, retain a
                # market-calibrated best selection rather than fabricating a
                # league-prior +EV long-shot.  The selector heavily weights
                # de-vigged market consensus in that case.
            ...
            row = {
                ...
                "warning": _join_warning("自動更新／尚未人工校正", _risk_warning(risk),
                    None if _has_reliable_team_inputs(model) else "球隊歷史資料仍在補齊：主推暫以市場校準與現有資料綜合計算",
        ...
        with self._db() as conn:
            conn.execute("""INSERT INTO football_model_versions VALUES(?,?,?,?,?,?)
              ON CONFLICT(model_version) DO UPDATE SET sources_json=excluded.sources_json,
                ...
                (FOOTBALL_MODEL_VERSION, None, json.dumps(["API-Football 賽程／球隊統計／聯賽強度", "football-data.org 近期賽果", "ESPN", "The Odds API"]),
    ...
    def _fetch_api_football_fixtures(self, target: date, seasons: Mapping[str, int]) -> list[dict[str, Any]]:
        ...
        for league_key, league_id in DEFAULT_LEAGUES.items():
            ...
            attempted += 1
            try:
                # ClubElo's public endpoint is intermittently unavailable
                # (HTTP 502).  Use the configured, authenticated API-Football
                # league standings as the durable pre-match strength fallback
                # instead.  This is fetched once per league, never per member.
                standings = self._fetch_league_standings(headers, league_id, season)
                response = requests.get(f"{self.config.api_football_base}/fixtures", headers=headers,
            ...
            for item in response.json().get("response", []):
                ...
                out.append({"event_id": str(fixture["id"]), "league_key": league_key,
                    ...
                    "standing_profile": {"home": standings.get(_team_key(teams["home"]["name"]), {}),
                                         "away": standings.get(_team_key(teams["away"]["name"]), {})},
                    "api_source": "API-Football"})
        if attempted and attempted == failures:
            raise AutoSnapshotDiagnosticError("external_api_error", "外部足球賽程資料暫時無法取得")
        return out

    def _fetch_league_standings(self, headers: Mapping[str, str], league: int, season: int) -> dict[str, dict[str, Any]]:
        """Return a team-strength profile from API-Football standings.

        Standings are an authoritative provider-side aggregate and give the
        model team-specific rates even where the optional /teams/statistics
        endpoint is unavailable under the current provider plan.  A failure is
        intentionally non-fatal: fixture/market snapshot creation continues.
        """
        try:
            response = requests.get(f"{self.config.api_football_base}/standings", headers=headers,
                params={"league": league, "season": season}, timeout=12)
            if not response.ok:
                return {}
            blocks = response.json().get("response") or []
            standings = (((blocks[0] if blocks else {}).get("league") or {}).get("standings") or [[]])
            rows = standings[0] if standings else []
            profiles: dict[str, dict[str, Any]] = {}
            for row in rows:
                team = (row.get("team") or {}).get("name")
                all_record = row.get("all") or {}
                played = int(all_record.get("played") or 0)
                if not team or played < 1:
                    continue
                goals = all_record.get("goals") or {}
                scored, conceded = float(goals.get("for") or 0), float(goals.get("against") or 0)
                points = float(row.get("points") or 0)
                # Rating remains bounded so a newly promoted side cannot make
                # the Poisson forecast explode, while its actual scoring and
                # conceding rates remain available to the model below.
                rating = 1650.0 + max(-150.0, min(150.0,
                    (points / played - 1.35) * 80.0 + (scored - conceded) / played * 25.0))
                profiles[_team_key(team)] = {"attack": round(scored / played, 4),
                    "defence": round(conceded / played, 4), "matches_used": played,
                    "rating": round(rating, 2), "points": points, "source": "API-Football standings"}
            return profiles
        except Exception:
            self._source_diagnostics["API-Football 聯賽強度"] = "聯賽排名資料暫時無法取得；將改用已保存歷史或球隊統計"
            return {}

    def _fetch_football_data_profiles(self, target: date) -> dict[str, dict[str, Any]]:
        """Build independent recent-form profiles from football-data.org.

        This is a second provider, not a mirror of API-Football.  It uses only
        completed matches before the Taiwan operating date, weighted toward
        recent matches.  No provider payload is exposed to Core or members.
        """
        if not self.config.football_data_key:
            self._source_diagnostics["football-data.org"] = "未設定金鑰；使用已保存歷史與 API-Football 資料"
            return {}
        raw: dict[str, list[tuple[datetime, int, int]]] = {}
        headers = {"X-Auth-Token": self.config.football_data_key}
        date_from = (target - timedelta(days=370)).isoformat()
        date_to = (target - timedelta(days=1)).isoformat()
        for league_key, competition in FOOTBALL_DATA_COMPETITIONS.items():
            try:
                response = requests.get(f"{self.config.football_data_base}/competitions/{competition}/matches",
                    headers=headers, params={"dateFrom": date_from, "dateTo": date_to, "status": "FINISHED"}, timeout=15)
                if not response.ok:
                    continue
                for match in response.json().get("matches", []):
                    ...
                    when = _parse_timestamp(match.get("utcDate"))
                    if home_name and away_name and when and isinstance(home_goals, int) and isinstance(away_goals, int):
                        raw.setdefault(_team_key(home_name), []).append((when, home_goals, away_goals))
                        raw.setdefault(_team_key(away_name), []).append((when, away_goals, home_goals))
            except Exception:
                self._source_diagnostics["football-data.org"] = "近期賽果資料暫時無法取得；使用已保存歷史與 API-Football 資料"
                continue
        profiles: dict[str, dict[str, Any]] = {}
        for team, matches in raw.items():
            recent = sorted(matches, key=lambda item: item[0], reverse=True)[:12]
            if len(recent) < 4:
                continue
            weights = [1.0 / (1.0 + .12 * index) for index in range(len(recent))]
            total = sum(weights)
            profiles[team] = {"attack": round(sum(item[1] * weight for item, weight in zip(recent, weights)) / total, 4),
                              "defence": round(sum(item[2] * weight for item, weight in zip(recent, weights)) / total, 4),
                              "matches_used": len(recent), "source": "football-data.org recent form"}
        if not profiles:
            self._source_diagnostics.setdefault("football-data.org", "未取得可配對近期賽果；使用已保存歷史與 API-Football 資料")
        return profiles
        ...
        for league, sport_key in keys.items():
            try:
                ...
                if isinstance(exc, FeedError):
                    self._last_odds_diagnostics.append({"league": league, "code": exc.code})
                    if exc.code in {"auth_failed", "quota_failed"}:
                        break
                continue
        return out

    # -------------------------- Base Model: football only ---------------------
    def _build_base_model(self, event: Mapping[str, Any], elo: Mapping[str, float],
                          history: Optional[Mapping[str, Mapping[str, Any]]] = None) -> dict[str, Any]:
        ...
        history = history or {}
        home_history, away_history = dict(history.get("home") or {}), dict(history.get("away") or {})
        independent = event.get("independent_profile") or {}
        home_independent, away_independent = dict(independent.get("home") or {}), dict(independent.get("away") or {})
        standing = event.get("standing_profile") or {}
        home_standing, away_standing = dict(standing.get("home") or {}), dict(standing.get("away") or {})
        # A durable, team-specific historical profile takes precedence over a
        # provider aggregate.  It is built from completed fixtures only and is
        # never replaced by the league-wide 1.40/1.10 fallback.
        if int(home_history.get("matches_used", 0)) >= 6:
            home_attack, home_defence = float(home_history["attack"]), float(home_history["defence"])
            home_quality.update({"quality": "historical_team_profile", "history": home_history,
                                 "shrinkage": .80, "fallback_reason": []})
        else:
            home_quality["history"] = home_history
            if int(home_independent.get("matches_used", 0)) >= 4:
                home_attack, home_defence = float(home_independent["attack"]), float(home_independent["defence"])
                home_quality.update({"quality": "independent_recent_form", "independent": home_independent,
                                     "shrinkage": .75, "fallback_reason": []})
            elif int(home_standing.get("matches_used", 0)) >= 5:
                home_attack, home_defence = float(home_standing["attack"]), float(home_standing["defence"])
                home_quality.update({"quality": "api_football_standings_profile", "standing": home_standing,
                                     "shrinkage": .60, "fallback_reason": []})
        if int(away_history.get("matches_used", 0)) >= 6:
            away_attack, away_defence = float(away_history["attack"]), float(away_history["defence"])
            away_quality.update({"quality": "historical_team_profile", "history": away_history,
                                 "shrinkage": .80, "fallback_reason": []})
        else:
            away_quality["history"] = away_history
            if int(away_independent.get("matches_used", 0)) >= 4:
                away_attack, away_defence = float(away_independent["attack"]), float(away_independent["defence"])
                away_quality.update({"quality": "independent_recent_form", "independent": away_independent,
                                     "shrinkage": .75, "fallback_reason": []})
            elif int(away_standing.get("matches_used", 0)) >= 5:
                away_attack, away_defence = float(away_standing["attack"]), float(away_standing["defence"])
                away_quality.update({"quality": "api_football_standings_profile", "standing": away_standing,
                                     "shrinkage": .60, "fallback_reason": []})
        league_home, league_away = _league_goal_prior(event["league_key"])
        home_elo = float(home_standing.get("rating", elo.get(_team_key(event["home"]), 1650.0)))
        away_elo = float(away_standing.get("rating", elo.get(_team_key(event["away"]), 1650.0)))
        ...
        strength_matched = bool(home_standing and away_standing) or (_team_key(event["home"]) in elo and _team_key(event["away"]) in elo)
        ...
        if not strength_matched:
            fallback_reasons.append("API-Football 聯賽強度未取得：使用中性強度值")
        quality = {"home_stats": bool(home_stats) or int(home_history.get("matches_used", 0)) >= 6 or int(home_independent.get("matches_used", 0)) >= 4 or int(home_standing.get("matches_used", 0)) >= 5,
                   "away_stats": bool(away_stats) or int(away_history.get("matches_used", 0)) >= 6 or int(away_independent.get("matches_used", 0)) >= 4 or int(away_standing.get("matches_used", 0)) >= 5,
                   "strength_rating": strength_matched,
                   ...
                   "confidence": _model_confidence(home_quality, away_quality, strength_matched, lineups_confirmed)}
        ...
        return {"model_version": FOOTBALL_MODEL_VERSION,
                ...
                "notes": "V2.1：API-Football 賽前資料、聯賽強度與已保存歷史；未取得可驗證 shot-based xG 時使用球隊進球／失球資料"}
    ...
    def refresh_fixture_readiness(self, date_str: str, event_id: str) -> dict[str, Any]:
        ...
        # Team strength and form remain frozen from the daily run; this call
        # updates volatile availability only, preserving the audit trail.
        old_model = self._load_model(date_str, str(event_id))
        model = self._build_base_model(event, {})
        model["quality"]["strength_rating"] = old_model.get("quality", {}).get("strength_rating", model["quality"].get("strength_rating", False))
        with self._db() as conn:
    ...
    def apply_manual_calibration(self, date_str: str, event_id: str,
        ...
        results = self._calculate_best_recommendations(model, normalised)
        with self._db() as conn:
    ...
    def _calculate_best_recommendations(self, model: Mapping[str, Any], markets: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
        """Return one risk-adjusted best selection, not a list of raw +EV tips.

        The selected row may have modestly negative EV when it is the clearly
        most robust settlement profile.  +EV remains visible, but cannot by
        itself promote a long-shot whose probability conflicts with market and
        historical team evidence.
        """
        saved = [dict(m) for m in markets]
        matrix = _score_matrix(float(model["lambda_home"]), float(model["lambda_away"]), rho=-0.08)
        fair = _devig_market_probabilities(saved)
        quality = model.get("quality", {}) if isinstance(model, Mapping) else {}
        model_weight = _fusion_model_weight(float(quality.get("confidence", .35)), quality)
        results: list[dict[str, Any]] = []
        for market in saved:
            raw_probability, raw_ev = _matrix_market_value(matrix, market)
            market_probability = fair.get(_market_key(market))
            if market["market_type"] == "moneyline" and market_probability is not None:
                final_probability = model_weight * raw_probability + (1 - model_weight) * market_probability
                final_ev = final_probability * float(market["decimal_price"]) - 1
            else:
                # For Asian settlement, preserve the exact win/half-win/push
                # payoff calculation and shrink the model-only edge toward the
                # de-vigged market rather than pretending it is a 1X2 chance.
                final_probability = raw_probability
                final_ev = raw_ev * model_weight if market_probability is not None else raw_ev
            non_loss = final_probability if market["market_type"] == "moneyline" else _market_non_loss_probability(matrix, market)
            agreement = 1.0 if market_probability is None else max(0.0, 1.0 - abs(raw_probability - market_probability) / .45)
            selection_score = .65 * non_loss + .20 * ((math.tanh(final_ev * 2.0) + 1.0) / 2.0) + .15 * agreement
            results.append({**market, "model_probability": raw_probability, "implied_probability": 1 / float(market["decimal_price"]),
                            "de_vig_market_probability": market_probability, "final_probability": final_probability,
                            "non_loss_probability": non_loss, "ev": final_ev, "final_ev": final_ev,
                            "model_weight": model_weight, "market_weight": 1 - model_weight,
                            "selection_score": selection_score, "playable": False, "label": "候選盤口"})
        # The member table contains three independent decisions.  Do not hide
        # valid moneyline and total calculations just because a spread had the
        # strongest score across the whole match.
        for market_type in ("moneyline", "spread", "total"):
            candidates = [item for item in results if item["market_type"] == market_type]
            if not candidates:
                continue
            winner = max(candidates, key=lambda item: (float(item["selection_score"]), float(item["ev"])))
            winner["playable"] = True
            category = {"moneyline": "獨贏", "spread": "讓分", "total": "大小"}[market_type]
            winner["label"] = (f"主推｜{category}價值盤" if float(winner["ev"]) >= self.config.min_ev
                               else f"主推｜{category}穩健盤（EV 非主要優勢）")
        return results
        ...
        for row in rows:
            ...
            if key not in grouped:
                ...
                record = grouped[key] = {
                    ...
                    "forecast": _saved_forecast(model, _score_decision_forecast(model, current)),
                    "risk": _risk_display(risk), "risk_display": _risk_display(risk),
...
def _parse_timestamp(value: Any) -> Optional[datetime]:
    try:
        ...
        return parsed if parsed.tzinfo else parsed.replace(tzinfo=TZ_UTC)
...
def _football_archived_recommendation_rows(conn: sqlite3.Connection, date_str: str) -> list[dict[str, Any]]:
    """Recover the newest valid pre-match row for every event."""
    values: list[str] = []
    current = conn.execute(
        "SELECT payload_json FROM football_automatic_snapshots WHERE date_str=?", (date_str,)
    ).fetchone()
    if current:
        values.append(current["payload_json"])
    values.extend(row["payload_json"] for row in conn.execute(
        """SELECT payload_json FROM football_automatic_snapshot_history
           WHERE date_str=? AND status='valid' ORDER BY id DESC""", (date_str,)
    ).fetchall())
    recovered: dict[str, dict[str, Any]] = {}
    for value in values:
        rows = _decode_snapshot_payload(value)["rows"]
        for row in rows:
            event_id = str(row.get("event_id") or "")
            if event_id and event_id not in recovered and row.get("recommendations"):
                recovered[event_id] = dict(row)
    return list(recovered.values())

def _recommendation_market_types(row: Mapping[str, Any]) -> set[str]:
    return {str(item.get("market_type")) for item in (row.get("recommendations") or ())
            if isinstance(item, Mapping) and item.get("market_type")}

def _append_snapshot_warning(row: dict[str, Any], message: str) -> None:
    existing = str(row.get("warning") or "")
    if message not in existing:
        row["warning"] = "｜".join(value for value in (existing, message) if value)

def _merge_football_automatic_rows(
    archived_rows: Iterable[Mapping[str, Any]],
    current_rows: Iterable[Mapping[str, Any]],
    attempted_at: str,
) -> list[dict[str, Any]]:
    """Freeze pre-match decisions at kickoff and retain partial market gaps."""
    archived = {str(row.get("event_id") or ""): dict(row) for row in archived_rows
                if row.get("event_id") and row.get("recommendations")}
    ...
    for source in current_rows:
        ...
        seen.add(event_id)
        previous = archived.get(event_id)
        if not previous:
            output.append(current)
            continue
        kickoff = _parse_timestamp(current.get("kickoff") or previous.get("kickoff"))
        if now and kickoff and kickoff <= now:
            locked = dict(previous)
            settlement = str(current.get("settlement_status") or "pending")
            if settlement != "pending":
                locked["settlement_status"] = settlement
            locked["record_state"] = "pregame_locked"
            _append_snapshot_warning(locked, "賽事已開賽：顯示開賽前最後保存的推薦與盤口")
            output.append(locked)
            continue
        previous_types = _recommendation_market_types(previous)
        current_types = _recommendation_market_types(current)
        missing = previous_types - current_types
        if missing:
            merged = list(current.get("recommendations") or ())
            merged.extend({**item, "data_status": "historical_reference", "is_current": False,
                           "observed_at": item.get("observed_at") or previous.get("updated_at"),
                           "display": "歷史參考｜" + str(item.get("display") or item.get("selection") or "")}
                          for item in (previous.get("recommendations") or ())
                          if isinstance(item, Mapping) and item.get("market_type") in missing)
            current["recommendations"] = merged
            current["first_market"] = previous.get("first_market") or current.get("first_market")
            if not current_types:
                current["latest_market"] = previous.get("latest_market")
                current["market_change"] = previous.get("market_change")
            current["record_state"] = "last_valid_market_retained"
            _append_snapshot_warning(current, "本次盤口資料不完整：缺少類別保留上一個有效快照，僅供歷史參考，不代表目前可下注價格")
        output.append(current)
    for event_id, previous in archived.items():
        if event_id in seen:
            continue
        kickoff = _parse_timestamp(previous.get("kickoff"))
        if now and kickoff and kickoff <= now:
            locked = dict(previous)
            locked["record_state"] = "pregame_locked"
            _append_snapshot_warning(locked, "賽事已開賽：顯示開賽前最後保存的推薦與盤口")
            output.append(locked)
    output.sort(key=lambda row: (str(row.get("kickoff") or ""), str(row.get("event_id") or "")))
    return output
...
def _sources_display(summary: Mapping[str, Any]) -> str:
    labels = (
        ...
        ("espn_events", "ESPN", "場"),
        ("football_data_teams", "football-data.org", "隊"),
        ("standing_strength_teams", "API-Football 聯賽強度", "隊"),
...
def _team_key(value: Any) -> str:
    ...
    # club suffixes.  Removing combining accents first keeps provider, ESPN and
    ...
    aliases = {
        ...
        "rcdmallorca": "mallorca", "mallorca": "mallorca",
        "realoviedo": "oviedo", "oviedo": "oviedo",
    }
...
def _model_confidence(home: Mapping[str, Any], away: Mapping[str, Any], strength_rating: bool, confirmed_lineups: bool) -> float:
    score = .25 + .25 * min(home.get("shrinkage", 0), away.get("shrinkage", 0))
    score += .20 if strength_rating else 0
...
def _has_reliable_team_inputs(model: Mapping[str, Any]) -> bool:
    ...
    聯賽強度與先發名單可提高信心，但接近開賽時仍可能暫時無法取得
...
def _score_decision_forecast(model: Mapping[str, Any], markets: Iterable[Mapping[str, Any]]) -> dict[str, Any]:
    """Choose one useful pre-match score without changing the Base Model.

    Exact-score modes often collapse to 1:1 because they optimise one cell in
    isolation.  This display decision first fuses the saved 1X2 market with the
    Base Model, then uses complete spread/total pairs as secondary constraints.
    The probability shown remains the honest probability of the selected cell;
    it is never converted into a confidence claim.
    """
    home_lambda = float(model.get("lambda_home", model.get("model_lambda_home", 0.0)) or 0.0)
    away_lambda = float(model.get("lambda_away", model.get("model_lambda_away", 0.0)) or 0.0)
    matrix = _score_matrix(home_lambda, away_lambda, rho=-0.08)
    saved = [dict(item) for item in markets]
    fair = _devig_market_probabilities(saved)
    ...
    quality = model.get("quality", {}) if isinstance(model, Mapping) else {}
    model_weight = _fusion_model_weight(float(quality.get("confidence", .35)), quality)
    result_probability: dict[str, float] = {}
    for side in ("home", "draw", "away"):
        market_probability = fair.get(("moneyline", side, None))
        result_probability[side] = (
            model_result[side] if market_probability is None
            else _logit_blend(model_result[side], market_probability, model_weight)
        )
    total_result = sum(result_probability.values()) or 1.0
    result_probability = {key: value / total_result for key, value in result_probability.items()}
    primary_result = max(result_probability, key=result_probability.get)

    # A decisive 1X2 direction is the primary constraint.  A near three-way
    # tie is left unforced so the model distribution can speak for itself.
    ranked_results = sorted(result_probability.values(), reverse=True)
    result_is_decisive = ranked_results[0] - ranked_results[1] >= .035
    candidate = np.ones(matrix.shape, dtype=bool)
    if result_is_decisive:
        candidate &= (home > away if primary_result == "home" else home < away if primary_result == "away" else home == away)

    projected_total = home_lambda + away_lambda
    total_pairs: list[tuple[float, float, float]] = []
    for line in sorted({round(abs(float(item["line"])), 2) for item in saved
                        if item.get("market_type") == "total" and item.get("line") is not None}):
        over = fair.get(("total", "over", line))
        under = fair.get(("total", "under", line))
        if over is not None and under is not None:
            total_pairs.append((line, over, under))
    total_direction = "neutral"
    if total_pairs:
        line, over_probability, under_probability = min(total_pairs, key=lambda row: abs(row[0] - projected_total))
        if max(over_probability, under_probability) >= .57:
            total_direction = "over" if over_probability > under_probability else "under"
            total_mask = (home + away > line) if total_direction == "over" else (home + away < line)
            if np.any(candidate & total_mask):
                candidate &= total_mask

    spread_direction = "neutral"
    spread_pairs: list[tuple[float, float, float]] = []
    for line in sorted({round(abs(float(item["line"])), 2) for item in saved
                        if item.get("market_type") == "spread" and item.get("line") is not None}):
        home_probability = fair.get(("spread", "home", line))
        away_probability = fair.get(("spread", "away", line))
        if home_probability is not None and away_probability is not None:
            spread_pairs.append((line, home_probability, away_probability))
    if spread_pairs:
        line, home_probability, away_probability = min(spread_pairs, key=lambda row: row[0])
        if max(home_probability, away_probability) >= .58:
            spread_direction = "home" if home_probability > away_probability else "away"
            relevant = next((item for item in saved if item.get("market_type") == "spread"
                             and item.get("side") == spread_direction
                             and round(abs(float(item.get("line") or 0)), 2) == line), None)
            if relevant is not None:
                values = home - away if spread_direction == "home" else away - home
                spread_mask = values + float(relevant["line"]) > 0
                if np.any(candidate & spread_mask):
                    candidate &= spread_mask

    ranked_matrix = np.where(candidate, matrix, -1.0)
    selected = _matrix_mode(ranked_matrix)
    probability = float(matrix[selected])
    sources = ["Base Model xG", "比分分布"]
    if any(key[0] == "moneyline" for key in fair): sources.append("1X2 去水市場")
    if total_pairs: sources.append("大小盤")
    if spread_pairs: sources.append("讓分盤")
    return {
        "exact_score_mode": f"{selected[0]}:{selected[1]}",
        "exact_score_probability": probability,
        "score_decision_version": SCORE_DECISION_VERSION,
        "score_decision_basis": "＋".join(sources),
        "score_result_direction": primary_result if result_is_decisive else "balanced",
        "score_total_direction": total_direction,
        "score_spread_direction": spread_direction,
        "score_uncertainty": "正確比分為低機率單一事件；此值是主預測，不是保證命中。",
    }
...
def _market_non_loss_probability(matrix: np.ndarray, market: Mapping[str, Any]) -> float:
    """Probability of a selection not producing a loss (wins plus pushes).

    Split Asian lines receive half credit for a half-win/half-push outcome, so
    the displayed figure remains comparable without disguising settlement.
    """
    home, away = np.indices(matrix.shape)
    if market["market_type"] == "moneyline":
        settled = home > away if market["side"] == "home" else away > home if market["side"] == "away" else home == away
        return float(matrix[settled].sum())
    values = home - away if market["market_type"] == "spread" and market["side"] == "home" else away - home if market["market_type"] == "spread" else home + away if market["side"] == "over" else -(home + away)
    handicap = float(market["line"]) if market["market_type"] == "spread" else -float(market["line"]) if market["side"] == "over" else float(market["line"])
    outcomes = []
    for leg in _asian_parts(handicap):
        outcomes.append(np.sign(values + leg) >= 0)
    return float((np.mean(np.stack(outcomes), axis=0) * matrix).sum())
...
def _fusion_model_weight(confidence: float, quality: Mapping[str, Any]) -> float:
    penalty = .10 if not quality.get("strength_rating") else 0
...
def _fusion_reason(model_weight: float, quality: Mapping[str, Any]) -> str:
    if model_weight <= .30: return "資料品質／先發／聯賽強度不完整：提高市場權重"
...
def _saved_forecast(model: Mapping[str, Any], quant_forecast: Optional[Mapping[str, Any]] = None) -> dict[str, Any]:
    ...
    try:
        score_home, score_away = (int(part) for part in str(mode).split(":", 1))
        default_score_probability = float(matrix[score_home, score_away])
    except (TypeError, ValueError, IndexError):
        default_score_probability = None
    ...
    return {
        ...
        "away_xg": away_lambda,
        "score": str(mode),
        "score_probability": (audit or {}).get("exact_score_probability", default_score_probability),
        "score_decision_basis": (audit or {}).get("score_decision_basis", "Base Model xG＋比分分布"),
        "score_uncertainty": (audit or {}).get("score_uncertainty", "正確比分為低機率單一事件；此值是主預測，不是保證命中。"),
...
def _quant_forecast(model: Mapping[str, Any], matrix: np.ndarray,
                    recommendations: list[Mapping[str, Any]],
                    markets: Iterable[Mapping[str, Any]] = ()) -> dict[str, Any]:
    home, away = np.indices(matrix.shape)
    decision = _score_decision_forecast(model, markets)
    return {"model_version": FOOTBALL_MODEL_VERSION, "goal_semantics": model.get("goal_semantics"),
            ...
            "exact_score_mode": decision["exact_score_mode"],
            "exact_score_probability": decision["exact_score_probability"],
            "score_decision_version": decision["score_decision_version"],
            "score_decision_basis": decision["score_decision_basis"],
            "score_result_direction": decision["score_result_direction"],
            "score_total_direction": decision["score_total_direction"],
            "score_spread_direction": decision["score_spread_direction"],
            "score_uncertainty": decision["score_uncertainty"],
            "score_distribution_method": model.get("score_distribution_method"),
...
def _assess_risk(event: Mapping[str, Any], quality: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not quality.get("strength_rating"):
        reasons.append("API-Football 聯賽強度未完整取得，長期強度採中性備援值")
    if not event.get("espn_verified"):
        reasons.append("ESPN 未完成同場賽程交叉驗證")
    if has_confirmed_lineups and total_injuries <= 2 and quality.get("home_stats") and quality.get("away_stats") and quality.get("strength_rating"):
        light = "green"
...
def _consensus_event(league: str, event: Mapping[str, Any]) -> list[dict[str, Any]]:
    """Compatibility name: selects actual complete markets, no synthetic odds."""
    rows = []
    for key, market in select_markets(event, _team_key, football=True).items():
        for side, value in market["sides"].items():
            rows.append({"league": league, "kickoff": event.get("commence_time"),
                "home": event.get("home_team"), "away": event.get("away_team"),
                "market_type": {"h2h":"moneyline", "spreads":"spread", "totals":"total"}[key],
                "side": side, "line": value["line"], "price": value["price"],
                "provider": "The Odds API / " + market["bookmaker"],
                "provider_event_id": str(event.get("id") or ""),
                "observed_at": market["observed_at"], "source_count": 1})
    return rows
...
def _validate_market(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not math.isfinite(m["decimal_price"]) or m["decimal_price"] <= 1: raise ValueError("decimal_price must be greater than 1")
    if m["market_type"] != "moneyline":
        m["line"] = float(m["line"])
        _asian_parts(m["line"])
    else: m["line"] = None
...
def _validate_market_set(markets: list[Mapping[str, Any]]) -> None:
    ...
    moneyline = {side for market, side in by if market == "moneyline"}
    if moneyline and moneyline != {"home", "away", "draw"}:
        raise ValueError("moneyline requires home, draw and away")
