...
def _score_forecast_display(forecast: Mapping[str, Any]) -> str:
    score = str(forecast.get("score") or "尚未儲存，請重新建立快照")
    probability = _percent(forecast.get("score_probability"), missing="機率未儲存")
    return f"主預測 {score}\n該比分機率 {probability}"


def _market_pick(recommendations, market_type, home, away, evaluations=()):
