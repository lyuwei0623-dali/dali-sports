...
from durable_store import storage_label, StorageUnavailable
...
def services() -> AppServices:
    ...
    return compose_services(read_only=st.session_state.get("user_role") != "admin")
...
def _render_brand() -> None:
    ...
    with title_column:
        ...
        st.caption("V20 CLEAN｜資料完整性與賽前存檔版")
    st.divider()
    ...
    if st.button("套用已輸入盤口並發布 MLB", disabled=not pending):
        try:
            with st.spinner("正在依人工盤口重新運算並發布…"):
                calculated = MLBPreReleaseService(history_store=app.mlb_store).run(
                    selected, pending, calibrated_at=_taipei_now().isoformat())
...
def _admin_page(app: AppServices) -> None:
    ...
    st.info(storage_label())
    if not os.environ.get("DATABASE_URL", "").strip():
        st.warning("目前不是永久保存模式。上線前請依 README 設定 DATABASE_URL；更新程式前請先備份現有資料。")
    ...
    with st.expander("資料來源檢查與設定", expanded=False):
        ...
        if st.button("檢查資料連線", key="check_sources"):
            with st.spinner("正在檢查資料來源…"):
                st.session_state["source_health"] = check_sources(
                    os.environ.get("THE_ODDS_API_KEY", "").strip(),
                    os.environ.get("API_FOOTBALL_KEY", "").strip(),
                    os.environ.get("FOOTBALL_DATA_API_KEY", "").strip(), date_str)
        if "source_health" in st.session_state:
            st.dataframe(st.session_state["source_health"], hide_index=True, use_container_width=True)
        st.caption("The Odds API 金鑰 → THE_ODDS_API_KEY；API-Football 金鑰 → API_FOOTBALL_KEY；football-data.org 金鑰 → FOOTBALL_DATA_API_KEY。三者不同，也不是帳戶登入密碼。")
    mlb_tab, football_tab = st.tabs(["MLB 後台", "足球後台"])

    with mlb_tab:
        st.subheader("MLB 自動存取快照")
        st.caption("MLB 歷史資料庫保存官方已完賽資料與盤口歷程；首次建立後會納入自動與人工推薦。")
        if st.button("建立／更新 MLB 歷史資料（本賽季＋前一賽季）"):
            try:
                current_year = int(selected.year)
                with st.spinner("正在匯入 MLB 官方已完賽資料…"):
                    result = app.mlb_store.sync_historical_games(
                        MLBPreReleaseService().mlb, (current_year - 1, current_year))
                st.success(f"MLB 歷史資料已保存 {result['games_upserted']} 筆；完成賽季：{len(result['completed_seasons'])} 個。")
            except Exception:
                st.error("MLB 歷史資料暫時無法匯入；已保存資料不會遺失。請稍後再次執行。")
    ...
    with football_tab:
        ...
        st.caption("歷史資料庫會保存已完賽資料與盤口時間序列；首次建立可分批續傳，不影響會員端。")
        if st.button("建立／續傳足球歷史資料（本賽季＋前一賽季）"):
            try:
                current = {key: int(value) for key, value in app.seasons.items()}
                previous = {key: int(value) - 1 for key, value in app.seasons.items() if int(value) > 1900}
                with st.spinner("正在分批匯入已完賽歷史資料；可安全重複執行以續傳…"):
                    current_result = app.football.sync_historical_fixtures(current, max_pages=20)
                    previous_result = app.football.sync_historical_fixtures(previous, max_pages=20)
                calls = int(current_result["api_calls"]) + int(previous_result["api_calls"])
                rows = int(current_result["fixtures_upserted"]) + int(previous_result["fixtures_upserted"])
                st.success(f"歷史資料已保存 {rows} 筆；本次使用 {calls} 次資料查詢。未完成聯賽可再次按此按鈕續傳。")
            except Exception:
                st.error("歷史資料本批暫時無法完成；已保存的資料不會遺失。請確認 API-Football 連線與額度後再次執行。")
        if st.button("立即執行足球自動快照", type="primary"):
            try:
                ...
                st.error("足球自動快照執行失敗，APP 已保護會員頁不受影響。請在「資料來源檢查」確認 ESPN、API-Football 聯賽強度與 The Odds API。")
...
def main() -> None:
    ...
    try:
        ...
        st.error("永久資料庫暫時無法讀寫或尚未設定完成。已停止本次操作；請管理員檢查設定，勿重建或清空資料庫。")
    except Exception:
        st.error("資料尚未就緒或本次操作失敗。請管理員先確認資料庫與快照；此頁不顯示內部錯誤或金鑰。")
    st.divider()
